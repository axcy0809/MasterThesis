...
class BuidingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        start = np.random.randint(
            0, len(self.df[self.df["Unnamed: 0"] == idx]) - self.T - self.S)
        print(start)
        # sensor_number = str(self.df[self.df["Unnamed: 0"]==idx][["Unnamed: 0"]][start:start+1].values.item())
        # index_in = torch.tensor([i for i in range(start, start+self.T)])
        # index_tar = torch.tensor([i for i in range(start + self.T, start + self.T + self.S)])
        _input = torch.tensor(
            self.df["Humidity", "sin_hour", "cos_hour", "sin_day", "cos_day", "sin_month", "cos_month"])
        target = torch.tensor(
            self.df["Humidity", "sin_hour", "cos_hour", "sin_day", "cos_day", "sin_month", "cos_month"])
        ...
        _input[:, 0] = torch.tensor(scaler.transform(
            _input[:, 0].unsqueeze(-1)).squeeze(-1))
        target[:, 0] = torch.tensor(scaler.transform(
            target[:, 0].unsqueeze(-1)).squeeze(-1))
